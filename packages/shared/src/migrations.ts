/**
 * D1 schema migration system (shared between backend and admin-backend).
 *
...
export const migrations: Migration[] = [
  ...
  // Must run before the unique partial index so that stale '' values
  // don't cause a unique-constraint violation during index creation.
  {
    name: "015_normalize_license_hash_empty_to_null",
    sql: "UPDATE user_scores SET license_hash = NULL WHERE license_hash = ''",
  },

  // ── indexes ────────────────────────────────────────────────────────
  {
    name: "016_idx_license_hash",
    sql: "CREATE UNIQUE INDEX IF NOT EXISTS idx_user_scores_license_hash ON user_scores (license_hash) WHERE license_hash IS NOT NULL",
  },
  {
    name: "017_idx_licenses_status",
    sql: "CREATE INDEX IF NOT EXISTS idx_licenses_status ON licenses (status)",
  },
  {
    name: "018_idx_completed_tasks_user_ticket",
