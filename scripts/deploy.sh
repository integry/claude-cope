...
for var in CF_WORKER_NAME CF_D1_DATABASE_NAME CF_D1_DATABASE_ID CF_USAGE_KV_ID CF_QUOTA_KV_ID CF_RATE_LIMIT_KV_ID OPENROUTER_API_KEY CF_PAGES_PROJECT ALLOWED_ORIGINS; do
  if [[ -z "${!var:-}" ]]; then
...
append_route_block "${CF_WORKER_ROUTE:-}"
if [[ -n "${CF_WORKER_EXTRA_ROUTES:-}" ]]; then
  IFS=',' read -r -a extra_worker_routes <<< "$CF_WORKER_EXTRA_ROUTES"
  for pattern in "${extra_worker_routes[@]}"; do
    pattern="${pattern#"${pattern%%[![:space:]]*}"}"
    pattern="${pattern%"${pattern##*[![:space:]]}"}"
    append_route_block "$pattern"
  done
fi
...
if ! $FRONTEND_ONLY; then
  ...
  if [[ -n "${IP_HASH_PEPPER:-}" ]]; then
    echo "$IP_HASH_PEPPER" | (cd "$ROOT/apps/backend" && wrangler secret put IP_HASH_PEPPER --config "$WRANGLER_CFG")
  else
    (cd "$ROOT/apps/backend" && printf 'y\n' | wrangler secret delete IP_HASH_PEPPER --config "$WRANGLER_CFG") || true
  fi
  if [[ -n "${FREE_ACCOUNT_COOKIE_SECRET:-}" ]]; then
