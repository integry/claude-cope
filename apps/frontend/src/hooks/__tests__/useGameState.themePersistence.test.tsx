...
describe("useGameState theme persistence", () => {
  ...
  it("clears stale session-backed paid state when /me no longer finds the session profile", async () => {
    ...
    remountWithState(makeState({ isPro: true, hasSessionPro: true, proKey: undefined, proKeyHash: undefined }));
  ...
  it("preserves local license-backed paid state when /me no longer finds the session profile", async () => {
    vi.mocked(fetchSessionProfile).mockResolvedValueOnce({ found: false });
    remountWithState(makeState({ isPro: true, hasSessionPro: true, proKey: "pro-key", proKeyHash: "pro-hash" }));
  ...
  it("does not let buy-theme overwrite a newer optimistic equip selection", async () => {
    remountWithState(makeState({ activeTheme: "default", economy: { currentTD: 12000, totalTDEarned: 12000, currentRank: "Junior Code Monkey", quotaPercent: 100, quotaLockouts: 0, tdMultiplier: 1 }, unlockedThemes: ["default", "amber"] }));
    ...
    act(() => { hookState.buyTheme("matrix"); });
    act(() => { hookState.setActiveTheme("matrix"); });
    expect(hookState.state.activeTheme).toBe("matrix");
    await act(async () => {
      buyRequest.resolve({ success: true, profile: createServerProfile({ active_theme: "default", unlocked_themes: ["default", "amber", "matrix"] }) });
      await buyRequest.promise;
    });
    expect(hookState.state.activeTheme).toBe("matrix");
    await act(async () => {
      updateRequest.resolve({ success: true, profile: createServerProfile({ active_theme: "matrix", unlocked_themes: ["default", "amber", "matrix"] }) });
  ...
  it("persists generator purchases through the paid session path without proKeyHash", async () => {
    const request = deferred<{ success: boolean; profile: ReturnType<typeof createServerProfile> }>();
    vi.mocked(buyGeneratorServer).mockReturnValueOnce(request.promise);
    act(() => { expect(hookState.buyGenerator("intern", 1)).toBe(true); });
    ...
    await act(async () => {
      request.resolve({ success: true, profile: createServerProfile({ current_td: 5900, total_td: 6000, inventory: { intern: 1 } }) });
      await request.promise;
    });
  ...
  it("persists upgrade purchases through the paid session path without proKeyHash", async () => {
    remountWithState(makeState({ inventory: { intern: 1 }, upgrades: [] }));
    const request = deferred<{ success: boolean; profile: ReturnType<typeof createServerProfile> }>();
    vi.mocked(buyUpgradeServer).mockReturnValueOnce(request.promise);
    act(() => { expect(hookState.buyUpgrade("intern-boost-copypaster")).toBe(true); });
    expect(buyUpgradeServer).toHaveBeenCalledWith("alice", "intern-boost-copypaster", undefined);
    await act(async () => {
      request.resolve({
        success: true,
        profile: createServerProfile({ current_td: 5000, total_td: 6000, inventory: { intern: 1 }, upgrades: ["intern-boost-copypaster"] }),
      });
      await request.promise;
    });
  ...
  it("persists achievement unlocks through the paid session path without proKeyHash", () => {
    ...
    act(() => { expect(hookState.unlockAchievement(achievementId)).toBe(true); });
  ...
  it("persists ticket progress updates through the paid session path without proKeyHash", () => {
    ...
    remountWithState(makeState({ activeTicket: { id: "T-1", title: "Ship it", sprintProgress: 1, sprintGoal: 3 } }));
    act(() => { hookState.updateTicketProgress(1); });
