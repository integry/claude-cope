...
function defaultFormatTimeAgo(isoString: string) {
  const now = Date.now();
...
export default function PartyFeedPanel({
  liveEvents,
  onClose,
  ...
  return (
    <div
      className={`fixed right-0 top-0 h-full w-80 bg-[#0d1117] border-l border-gray-700 flex flex-col z-20 ${className}`.trim()}
      style={style}
    >
