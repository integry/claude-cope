function HeaderBar({ rank, totalTDEarned, quotaPercent, outageHp, activeMultiplier }: { rank: string; totalTDEarned: number; quotaPercent: number; outageHp: number | null; activeMultiplier: number }) {
  return (
    <div className={`sticky top-0 z-10 border-b pb-2 mb-2 ${outageHp !== null ? "bg-red-900 border-red-500" : "bg-[#0d1117] border-green-800"}`}>
      <div className="flex items-center justify-between text-green-400 mb-1">
        ...
        <span><span className="text-gray-500">Technical Debt:</span> <span className="text-white font-bold">{totalTDEarned.toLocaleString()} TD</span>{activeMultiplier > 1 && <span className="text-yellow-400"> ({activeMultiplier.toFixed(1)}x)</span>}</span>
      </div>
