    ...
    <div className={`sticky top-0 z-10 border-b pt-3 pb-2 mb-2 flex gap-4 ${outageHp !== null ? "bg-red-900 border-red-500" : "bg-[#0d1117] border-gray-700"}`}>
      ...
      <div className="flex-1 flex items-start justify-between text-green-400">
        ...
        <span className="flex flex-col items-end">
          ...
          <span><span className="text-gray-500">Technical Debt:</span> <span className="text-white font-bold">{Math.floor(currentTD).toLocaleString()} TD</span>{activeMultiplier > 1 && <span className="text-yellow-400"> ({activeMultiplier.toFixed(1)}x)</span>}</span>
        </span>
      </div>
    </div>
  );
